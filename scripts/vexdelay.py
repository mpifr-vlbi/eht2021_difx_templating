...
class ClockDef():
	...
	@classmethod
	def from_csv_file(cls,start,stop,filename="delays.csv",offset=0.0,rate=0.0,granularity=0.0):
		clks = []
		prev_delay = 1e9
		with open(filename,"r") as fh:
			...
			for row in csv_reader:
				...
				delay = float(row[2])+offset
				if (start_min_r_stop_ctd.custom_total_seconds() < 0.0
				    and stop_min_r_start_ctd.custom_total_seconds() > 0.0
				    and abs(delay - prev_delay) > granularity):
					clks.append(cls(r_start,r_stop,delay,rate))
					prev_delay = delay
...
if __name__ == "__main__":
	...
	parser.add_argument("-g", "--clk-granularity", metavar="CLOCKGRANULARITY", type=float, default=0.0,
						help="smallest clock delta in microseconds to consider as a clock change")
	...
	clks = ClockDef.from_csv_file(start,stop,filename=args.delays_file,offset=args.clk_offset,rate=args.rate,granularity=args.clk_granularity)
	#~ for clk in clks:
